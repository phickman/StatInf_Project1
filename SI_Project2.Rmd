...
# supplement by dose
ggplot(ToothGrowth, aes(x = supp, y = len, fill = supp)) + geom_boxplot() + facet_wrap(~dose) + xlab("Supplement") + ylab("Tooth Length") + ggtitle("Length by Supplement and Dose")
```
...
```{r}
# dose by supplement
ggplot(ToothGrowth, aes(x = dose, y = len, fill = dose)) + geom_boxplot() + facet_grid(~supp) + xlab("Tooth Length") + ylab("Supplement") + ggtitle("Length by Dose and Supplement")
```
...
The tests for the null hypothesis is the supplement type and dose have no effect on tooth growth.

To compare the VC supplement against the three dosage levels we need to subset the data first and then test.
...
To compare the OJ supplement against the three dosage levels we need to subset the data first and then test
...
To compare the dosage to supplement we need to subset the data first and then test.
...
In conclusion, higher doses, no matter the supplement, is associated with greater tooth length.  However, OJ rather than VC is associated with greater tooth length for 0.5 and 1 mg/d dose and is not sufficiently different at the 2 mg/d level.
